...
const definitions = {
  "landing": "Una landing page es una página web diseñada específicamente para una campaña de marketing o promoción. Su objetivo principal es convertir a los visitantes en clientes potenciales o clientes, generalmente a través de un formulario de registro o compra. Suelen ser páginas de destino únicas y no vinculadas a la navegación general del sitio web.",
...
const generateResponse = (userMessage) => {
    userMessage = userMessage.toLowerCase(); // Convert user message to lowercase for case-insensitive matching
    ...
    for (const category in categories) {
        ...
        if (checkKeywords(userMessage, keywords)) {
            ...
            if (category === "definitions") {
              ...
              const response = `Me alegra haber sido util.`;
              return response;
    ...
    // If no specific category was found, provide a default response
...
function checkKeywords(userMessage, keywordArray) {
    // Check if the user's message contains keywords from a specific category
...
function getDetectedKeyword(userMessage, keywordArray) {
    // Get the detected keyword from a specific category
